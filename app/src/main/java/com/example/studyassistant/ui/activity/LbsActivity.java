...
import com.example.studyassistant.ClusterBaiduItem;
import com.example.studyassistant.DemoApplication;
import com.example.studyassistant.R;
import com.example.studyassistant.bean.IconModel;
import com.example.studyassistant.bean.LBSModel;
import com.example.studyassistant.event.IconEvent;
import com.example.studyassistant.event.RequestDataEvent;
import com.example.studyassistant.job.IConJob;
import com.example.studyassistant.job.RequestLogicJob;
...
public class LbsActivity extends LbsBaseActivity {
    ...
    protected void initListeners() {
        ...
        mClusterManager.setOnClusterItemClickListener(new ClusterManager.OnClusterItemClickListener<ClusterBaiduItem>() {
            ...
            public boolean onClusterItemClick(ClusterBaiduItem item) {
                Toast.makeText(LbsActivity.this, item.getLBAModel().getTitle(), Toast.LENGTH_SHORT).show();
                IconClick(item);
                return true;
            }
        });
        ...
        mClusterManager.setOnClusterClickListener(new ClusterManager.OnClusterClickListener<ClusterBaiduItem>() {
            ...
            public boolean onClusterClick(Cluster<ClusterBaiduItem> cluster) {
                Toast.makeText(LbsActivity.this, "聚合图标：" + cluster.getSize(), Toast.LENGTH_SHORT).show();
    ...
    private void DownLoadIcons(List<ClusterBaiduItem> clusterBaiduItems) {
        ...
        //执行下载图标的job
        if (logoUrl.size() > 0) {
            IConJob iConJob = new IConJob(LbsActivity.this, logoUrl);
